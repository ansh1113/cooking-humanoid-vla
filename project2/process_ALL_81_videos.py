"""
PROCESS ALL 81 CURATED VIDEOS + PANEER TEST VIDEO
- Skips already processed videos
- Shows REAL cost estimate based on actual data
- Crash-proof with incremental saving
"""
import whisper
import json
import cv2
...
def main():
    print("="*70)
    print("🌍 PROCESSING ALL 81 DIVERSE VIDEOS + PANEER TEST")
    print("="*70)
    
    # 1. Load all videos from curated list
    try:
    ...
    # Add the original Paneer test video (if not already in list)
    ...
    categories = Counter(v['category'] for v in all_videos)
    for cat, count in sorted(categories.items()):
        print(f"   {cat:25s}: {count:2d} videos")
    ...
    if os.path.exists(OUTPUT_FILE):
        try:
            ...
            logger.info(f"\n🔄 Found {len(all_labeled_data)} existing labels from {len(processed_urls)} videos.")
        except:
            logger.warning("Output file corrupt. Starting fresh.")
    
    # Also check indian_veg file
    if os.path.exists('data/processed/indian_veg_universal_labels.json'):
        try:
            ...
            logger.info(f"🔄 Merged {len(indian_veg_data)} labels from indian_veg file")
        except:
            pass
    
    # Also check paneer test file
    if os.path.exists('paneer_labels.json'):
        try:
            ...
            logger.info(f"🔄 Merged {len(paneer_data)} labels from paneer test")
        except:
            pass
    
    # 3. Calculate what's left to do
    videos_to_do = [v for v in all_videos if v['url'] not in processed_urls]
    
    print(f"\n{'='*70}")
    print(f"📊 PROCESSING SUMMARY:")
    print(f"{'='*70}")
    print(f"Total videos in queue:     {len(all_videos)}")
    print(f"Already processed:         {len(processed_urls)}")
    print(f"Existing labels:           {len(all_labeled_data)}")
    print(f"Videos remaining:          {len(videos_to_do)}")
    ...
    # 4. ESTIMATE COST based on actual data
    # From 8 videos we got 64 labels
    # Average: 8 labels per video
    # Cost per label: ~$0.02
    estimated_labels = len(videos_to_do) * 8
    estimated_cost = estimated_labels * 0.02
    
    print(f"\n{'='*70}")
    print(f"💰 COST ESTIMATE (Based on actual data):")
    print(f"{'='*70}")
    print(f"Videos to process:         {len(videos_to_do)}")
    print(f"Avg labels/video:          8 (from test run)")
    print(f"Expected new labels:       ~{estimated_labels}")
    print(f"Cost per label:            $0.02")
    ...
    # Show final stats
    from collections import Counter
    labels = [d['label'] for d in all_labeled_data]
    categories = [d.get('category', 'unknown') for d in all_labeled_data]
    
    print(f"\n📊 FINAL STATISTICS:")
    print(f"{'='*70}")
    print(f"Total unique actions: {len(set(labels))}")
    print(f"\nTop 20 actions:")
    for i, (action, count) in enumerate(Counter(labels).most_common(20), 1):
