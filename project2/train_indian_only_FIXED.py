"""
Train on ONLY Indian cooking data - FIXED
"""
import torch
...
# Keep actions with 3+ samples (more conservative)
MIN_SAMPLES = 3
...
# Filter data to only common actions
filtered_data = [d for d in indian_data if d['label'] in common_actions]
for item in filtered_data:
    item['train_label'] = item['label']

print(f"📊 Filtered to {len(filtered_data)} samples (removed rare actions)")

# Check distribution
train_labels = Counter(d['train_label'] for d in filtered_data)
print(f"\n📈 Final distribution:")
for i, (action, count) in enumerate(train_labels.most_common(), 1):
...
print(f"💾 Model saved: models/indian_cooking_vla.pt")
print("="*60)
